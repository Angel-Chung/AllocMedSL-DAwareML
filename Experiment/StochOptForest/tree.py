...
def generate_candidate_splits(node_X, node_X_estimate, mtry, n_proposals, balancedness_tol, min_leaf_size, seed):
...
def build_tree(Y, X, Y_est, X_est, opt_solver = None, hessian_computer = None,
                         ...
                         crit_computer = None, impurity_computer = None, 
    ...
    tree_temp =  tree(opt_solver = opt_solver, hessian_computer = hessian_computer,
                         ...
                         compute_update_step = compute_update_step, impurity_computer = impurity_computer,
...
#                 (valid_side, valid_side_est, valid_dim_proposals, valid_thr_proposals) = generate_candidate_splits(node_X, node_X_estimate, self.mtry, n_proposals, self.balancedness_tol, self.min_leaf_size, self.seed)
...
#                 # (imp_parent, imp_left, imp_right, imp_dec) = impurity_computer(node_Y, left_split_sample_inds, right_split_sample_inds)
#                 # node.imp_parent = imp_parent; node.imp_dec = imp_dec
#                 # node.imp_left = imp_left; node.imp_right = imp_right
...
class tree:
    def __init__(self,
                 opt_solver = None, compute_update_step = None, 
                 gradient_computer = None, hessian_computer = None,
                 crit_computer = None, search_active_constraint = None, impurity_computer = None, 
                 honesty = False,
                 min_leaf_size=10,
                 max_depth=10,
                 n_proposals=200,
        ...
        self.crit_computer = crit_computer
        self.search_active_constraint = search_active_constraint
        self.impurity_computer = impurity_computer
        # tree parameters
        self.min_leaf_size = min_leaf_size
        self.max_depth = max_depth
        self.balancedness_tol = balancedness_tol
    ...
    def create_splits(self, Y, X, Y_est, X_est):
        ...
        while len(node_list) > 0:
            ...
            if node.split_sample_inds.shape[0] >= self.min_leaf_size and depth < self.max_depth:
                ...
                node_Y_left = Y[left_split_sample_inds];
                node_Y_right = Y[right_split_sample_inds];
                (imp_parent, imp_left, imp_right, imp_dec) = self.impurity_computer(node_Y, node_sol, node_Y_left, node_Y_right, 
                        node_h_left, node_h_right, f_grad, node_hessian, best_split_ind)
                node.imp_parent = imp_parent; node.imp_dec = imp_dec
                node.imp_left = imp_left; node.imp_right = imp_right
    ...
    def print_tree_rec(self, node):
        if not node:
            return
        print("Node: ({}, {}, {})".format(node.feature, node.threshold, node.imp_dec))
        print("Left Child")
    ...
    def find_impurity_decrease(self, node):
        if node.feature is -1:
            return []

        imp_list = [node.imp_dec] + self.find_impurity_decrease(node.left) + self.find_impurity_decrease(node.right)
...
class forest:
            ...
            try:
                if not self.honesty:
                    ...
                    tree = build_tree(Y_train, X_train, Y_est, X_est, opt_solver = self.opt_solver, compute_update_step = self.compute_update_step,
                                ...
                                impurity_computer = self.impurity_computer, 
                                honesty = self.honesty, mtry = self.mtry,
                                min_leaf_size = self.min_leaf_size, max_depth = self.max_depth, 
                                n_proposals = self.n_proposals, balancedness_tol = self.balancedness_tol, verbose = self.verbose, seed = self.seed)
                if self.honesty:
                ...
                print("tree exception: tree=None")
                tree = None
            print(tree)
            trees.append(tree)
        ...
        if not self.honesty:
            ...
            for t, tree in enumerate(self.trees):
                ...
                if tree is not None:
                    ...
                    #print(len(leaf.est_sample_inds))
    ...
    def compute_impurity_fi(self, p):

        fi = np.zeros((len(self.trees), p))
        for i in range(len(self.trees)):
            tree = self.trees[i]
            feature_list = tree.find_feature(tree.tree)
            imp_list = tree.find_impurity_decrease(tree.tree)
            for j in range(len(feature_list)):
                fi[i, feature_list[j]] += imp_list[j]
        return fi.mean(0) 
