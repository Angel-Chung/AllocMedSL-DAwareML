...
def solve_SL(Y, A_mat = None, b_vec = None, alpha = None, verbose = False, weights = None, if_weight = False):

    #L = Y.shape[1]
    #n = Y.shape[0]
    n_facilities=Y.shape[0]
    if not if_weight:
        weights = np.ones(n_facilities)/n_facilities

    allocation = cp.Variable(shape=(n_facilities), name="allocation")
    constraints = [allocation >= 0, cp.sum(allocation*weight) <= allocation_max, loss >= 0]
    objective = cp.Minimize(cp.sum(np.maximum(allocation*weight - Y, 0.0)))
    # solution
    prob = cp.Problem(objective, constraints)
    prob.solve(cp.SCIPY, scipy_options={"method": "highs"})
    decision=allocation.value
    objective=objective.value
    lam_de=0
    lam_st=0

    return (decision, lam_de, lam_st, objective)


def solve_multi_nvTRY(Y,h_list = None, b_list = None, C = None, verbose = False, if_weight = False, weights = None,lbda=None):
    n_facilities=Y.shape[0]
    Y=Y[:,1]
    decision=[]
    obj=[]
    for i in Y:     
        ...
        constraints = [allocation >= 0,loss >= 0,loss >= Y - lbda*allocation]        
        objective = cp.Minimize(cp.sum(loss))
        # solution
        prob = cp.Problem(objective, constraints)
        prob.solve(cp.SCIPY, scipy_options={"method": "highs"})
        allocationV=allocation.value    
        decision.append(allocationV)
        objectiveV=objective.value
        obj.append(objectiveV)
    lam_de=0
    lam_st=0
...
def compute_obj(decision):
       # obj = np.mean(np.matmul((decision[np.newaxis, :] - Y) * (decision - Y >= 0), h_list) + np.matmul((Y - decision[np.n>
    obj =np.sum(np.maximum(decision[np.newaxis, :] - Y, 0.0)-lbda*decision[np.newaxis, :])
    return obj

def solve_multi_nv(Y,h_list = None, b_list = None, C = None, verbose = False, if_weight = False, weights = None,lbda=None):
    if verbose:
        print("h_list: ", h_list)
        print("b_list: ", b_list)
        print("constraint: ", C)
    ...
    w = weights
    prop = lbda   # this is the lagrangian multiplier for the capacity constraint
    ...
    if sol_temp.sum() <= C:
        if verbose:
            ...
            print("solutionSum: ",sol_temp.sum())
...
def compare_forest_one_run(X, Y, X_est, Y_est, h_list = None, b_list = None, C = None,
    n_trees = 100, honesty = False, mtry = 5, verbose = False, subsample_ratio = 0.5,
    oracle = False,min_leaf_size=10, max_depth=100, n_proposals = 200, 
    balancedness_tol = 0.3, bootstrap = False, seed = None,lbda=None):
    models = {}

    opt_solver = partial(solve_multi_nv, h_list = h_list, b_list = b_list, C = C, verbose = verbose,lbda=lbda)
    #opt_solver = partial(solve_SL, verbose = verbose)
    hessian_computer = partial(compute_hessian, h_list = h_list, b_list = b_list, C = C)
    gradient_computer = partial(compute_gradient, h_list = h_list, b_list = b_list, C = C)
    active_constraint = partial(search_active_constraint, C = C)
    update_step = partial(compute_update_step, constraint = True)
    ...
    #for key in models.keys():
        #print(key)
    print("MODEL",models['rf_approx_sol'])

    #Nx_test = 143
    #for i in range(Nx_test):
     #   weights = models['rf_approx_sol'].get_weights(X_est[i, :])
    #print(weights)
    return models
...
def evaluate_one_run(models, X, Y, X_est, Y_est, Nx_test, Ny_train, Ny_test, h_list = None, b_list = None, C = None, verbose = False, seed = None,lbda=None):
    
    p = X_est.shape[1]
    L = Y_est.shape[1]

    solver = partial(solve_multi_nv, h_list = h_list, b_list = b_list, C = C, verbose = verbose,lbda=lbda)
    ...
    decisions = {}
    risk = {}
    #modelkey=['rf_approx_sol', 'rf_approx_risk', 'rf_rf', 'rf_grf', 'rf_oracle']
    modelkey=['rf_approx_sol']
    ...
    for i in range(X_test.shape[0]):
        ...
        for key in modelkey:
            ...
            (decisions[key][i, :], _, _, _) = solver(Y_test, weights = weights, if_weight = True)
...
def evaluate_risk_singleX(decision, Y_test, h_list, b_list):
    ...
    temp= Y_test-decision[np.newaxis, :]
    return np.sum(np.maximum(temp, 0.0))
